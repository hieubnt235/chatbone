...
class MCPServer(FastMCP):
    ...
    def __init__(
        ...
        *args,
        name: str | None = None,
        tools: list[types.AnyFunction] | None = None,
    ...
    async def _create_new_mcp(self, websocket: WebSocket):
        self.check_factory()
        mcp = self.__class__(
            tools=self._raw_tools, websocket=websocket, __IS_FACTORY__=False
        )
        try:
            ...
            logger.debug(
                f"Delete MCPServer executor with id: '{mcp.id}'.\n"
                f"Number of MCPServer executor now: {len(self._servers)}"
            )
    ...
    def app(self, mcp_path: str) -> FastAPI:
        ...
        async def mcp_endpoint(websocket: WebSocket):
            try:
                ...
                logger.info(
                    f"MCPServer App '{self.app_name}': Websocket endpoint connected.{"=" * 10}"
                )
                ...
                logger.info(
                    f"MCPServer App '{self.app_name}': Websocket endpoint disconnected.{"~" * 10}"
                )
            except Exception as e:
                logger.exception(e)
        ...
        async def states_capture(pid: int | Literal["current"]):
            ...
            data = dict(
                mcp_servers=dict(count=len(svs), list=svs),
                mcp_websockets=dict(count=len(wss), list=wss),
                tasks=dict(count=len(ts), list=ts),
        ...
        async def monitor_endpoint(websocket: WebSocket, pid: int | Literal["current"]):
            ...
            tasks = [
                asyncio.create_task(receive_text(0.001), name="dummy_receive_text"),
        ...
        app.add_api_websocket_route(
            mcp_path, mcp_endpoint, name=MCP_ENDPOINT_ROUTE_NAME
        )
        app.add_api_route("/_states/{pid}", states_capture, name="states_endpoint")
        app.add_api_websocket_route(
            "/_monitor/{pid}", monitor_endpoint, name="monitor_endpoint"
        )

        self._mcp_path = mcp_path
        self._app = app
        logger.info(
            f"MCPServer Application '{self.app_name}' created at PID {os.getpid()}."
        )
        return self._app
    ...
    async def _run(self):
        ...
        tasks = [
            asyncio.create_task(self._read_websocket(), name="read_websocket"),
        ...
        for task in done:
            logger.debug(
                f"Task {task.get_coro().__qualname__} is DONE. Exception={task.exception()}"
            )
        for task in pending:
            task.cancel()
            logger.debug(
                f"Task {task.get_coro().__qualname__} is CANCELED."
