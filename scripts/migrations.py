...
def run_migrations(alembic_ini_path: str, ini_section: str, script_location: str, message: str = "", upgrade_only=False,
                   upgrade_head=True):
	"""
...
if __name__ == "__main__":
	...
	parser.add_argument("-c", "--config", default=alembic_path.as_posix(),
	                    help="Absolute path to the alembic.ini file. Default is ALEMBIC_PATH in envars or \'../alembic.ini\'.")
	parser.add_argument("-s", "--script-location", default=alembic_path.parent.as_posix(),
	...
	parser.add_argument("--no-upgrade", action="store_true",
	                    help="Disable run 'alembic upgrade head' after creating a revision.")
	...
	run_migrations(alembic_ini_path=args.config, ini_section=args.ini_section, script_location=args.script_location,
	               message=args.message, upgrade_only=args.upgrade_only, upgrade_head=not args.no_upgrade)
