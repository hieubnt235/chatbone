...
JWTError = HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
...
class AuthenticationSVC:
	...
	def _encode_jwt(self, userinfo: UserInfoReturn)->str:
		return jwt.encode(userinfo.model_dump(exclude={'hashed_password'},mode='json'),
		                  key=self.secret_key,
	...
	def _decode_jwt(self,jwt_str: str) -> TokenInfoReturn:
		...
		if self.jwt_encode_algorithm is None:
			...
			algo=self.jwt_encode_algorithm if isinstance(self.jwt_encode_algorithm,Sequence) else [self.jwt_encode_algorithm]
