...
class ToolsClient:
	"""
	Open maximum 1 connection to each server.

	Client for MCPServer. This class leverage langchain and fastmcp.client to adapt the MCPServer.

	One client only have one cm. CM mean the open ws, so that calls in one cm cannot make in separated replica.

	Note that langchain tool must be called in the session that list tools. Does not flexible.
