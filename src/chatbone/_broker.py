...
userdata = {
	"chat_sessions":
		[
			{
				"id":"123",
				"messages":[{"role":"assistant", "content":"you are an assistant"},{"role":"user", "content": "i am user."}],
			},
			{
				"id"  : "456",
				"messages": [{"role": "assistant", "content": "you are an assistant"},{"role": "user", "content": "i am user."}],
...
class UserData(ChatBoneJsonModel):
		...
		async with AsyncExitStack() as stack:
			...
			for i in len(chat_sessions):
				if is_overridden[i] or (str(chat_sessions[i].id) not in current_session_keys):
					await pipeline.json().set(self.key(),f".chat_sessions.{}")
	...
	async def _create_chat_sessions(self, chat_sessions: list[ChatSessionData], redis_or_pipeline: Redis | None = None,
		...
		await self.db().json().set(self.key(),)
...
class Broker:
	...
	async def update_user_data(self, userdata: UserData) -> str:
