...
class AssistantApp(BaseModel):
	...
	@classmethod
	async def create(cls, assistant_name)->Self:
...
class ChatAssistantSVC(_DataSVC):
	...
	async def delete_chat_session(self,chat_session_id:UUID):
		return await self._call_data_svc_method(self._delete_chat_session,ChatSessionSVCDelete(token_id=self.token_id,
	...
	async def get_chat_session(self, chat_session_id: UUID)->ChatSessionData:
		...
		try:
			...
			logger.info(f"No chat session with id '{chat_session_id}' found in cache. Try to load from datastore.")
			messages = await self._call_data_svc_method(self._get_messages,
			                                            ChatSVCGetLatest(token_id=self.token_id,chat_session_id=chat_session_id))
			chat_summaries = await self._call_data_svc_method(self._get_chat_summaries,
			                                                  ChatSVCGetLatest(token_id=self.token_id,chat_session_id=chat_session_id,))
			def _extract():
				s = [s.summary for s in chat_summaries.summaries]
				m = [Message(role=m.role,content=m.content) for m in messages.messages]
				return {chat_session_id: ChatSessionData(id=chat_session_id,messages=m,summaries=s)}
	...
	async def _call_data_svc_method[S,T](self,func:Callable[[S],Coroutine[...,...,T]], schema:S)->T:
		...
		try:
			...
			if he.detail=="__TOKEN_ERROR__": # __TOKEN_ERROR__ is the special HTTP exception from datastore service
	...
	async def _re_authenticate(self):
		...
		token:TokenInfoReturn = (await AUTH.get_user(ClientRequestSchema(headers={"Authorization": f"Bearer {token_jwt.access_token}"}))).content.tokens[-1]
		user_token = UserToken.model_validate(token,from_attributes=True) # It's maybe redundant
		assert user_token.expires_at>utc_now() # Must, or reimplement auth/datastore service.
