...
async def test_create_token_id(random_user, chat_db_session, token_id):
	...
	token_info = await user_repo.authenticate(username=random_user.username, hashed_password=random_user.password,
...
def _fake_messages(n):
	return [dict(role=random.choice(["system", "user", "assistant"]), content=str(uuid7())) for _ in range(n)]
