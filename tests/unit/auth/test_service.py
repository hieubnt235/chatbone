...
async def test_auth_service(chat_db_session, random_user):
	...
	with pytest.raises(HTTPException) as e:
		await auth_service.authenticate(UserCredentials(username="xyz", hashed_password=user_cre.hashed_password))
	assert e.value == UsernameNotFoundError

	with pytest.raises(HTTPException) as e:
		await auth_service.authenticate(UserCredentials(username=user_cre.username, hashed_password="stuff"))
