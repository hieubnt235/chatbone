...
from auth.auth_svc import auth_svc
from utilities.schemas.auth import UserRegister, TokenJWT
...
async def authenticate(
    ...
    jwt = await auth_svc.authenticate(
        UserRegister(username=schema.username, password=schema.password)
    )
    return TokenJWT(access_token=jwt, token_type="bearer")
