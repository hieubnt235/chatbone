...
from utilities.schemas.auth import UserRegister
...
JWTError = HTTPException(
    status_code=status.HTTP_401_UNAUTHORIZED, detail="Cannot decode jwt."
)
...
class AuthenticationSVC:
    ...
    async def register(self, schema: UserRegister) -> str:
        ...
        """
        req = ClientRequestSchema[UserCreate](
            body=UserCreate(
        ...
        res: ClientResponseSchema[UserInfoReturn] = (
            await self.datastore.user.access.create(req)
        )
    ...
    async def authenticate(self, schema: UserRegister) -> str:
        ...
        res: ClientResponseSchema[UserInfoReturn] = (
            await self.datastore.user.access.verify(req)
        )
        # self.datastore.check_ok(res)
        return await asyncio.to_thread(self._encode_jwt, res.content)
    ...
    async def get_user(self, jwt: str) -> UserInfoReturn:
        ...
        try:
            ...
            req = ClientRequestSchema[Token](
                body=Token(token_id=token.id),
        ...
        res: ClientResponseSchema[UserInfoReturn] = (
            await self.datastore.user.access.get(req)
        )
        # self.datastore.check_ok(res)
        return res.content
    ...
    def _encode_jwt(self, userinfo: UserInfoReturn) -> str:
        return jwt.encode(
            userinfo.model_dump(exclude={"hashed_password"}, mode="json"),
            key=self.secret_key,
    ...
    def _decode_jwt(self, jwt_str: str) -> TokenInfoReturn:
        ...
        if self.jwt_encode_algorithm is None:
            ...
            algo = (
                ...
                if isinstance(self.jwt_encode_algorithm, Sequence)
                else [self.jwt_encode_algorithm]
            )
