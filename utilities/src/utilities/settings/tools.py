...
class ToolsClient:
    ...
    def __init__(
        ...
        *,
        ...
        if ws_urls_or_transports is not None:
            assert [
                isinstance(t, str) or isinstance(t, WSTransport)
                for t in ws_urls_or_transports.values()
            ]
    ...
    async def _create_transports_from_tools_app(
        self, base_tools_app_url: str
    ) -> list[WSTransport]:
        base_tools_app_url = base_tools_app_url.removesuffix("/")
        url = urlparse(base_tools_app_url)
        async with aiohttp.ClientSession() as session:
            async with session.get(
                url.geturl() + "/list_mcp_apps", raise_for_status=True
            ) as response:
    ...
    async def _connect_clients(self, mcp_apps: list[str] | str | None) -> Client:
        ...
        for app_name in mcp_apps:
            self._servername2clients[app_name] = (
                await self._aestack.enter_async_context(
    ...
    async def get_tools(
        self,
        mcp_apps: list[str] | str | None,
        *,
        timeout: float | None = 30.0,
        stop_when_one_fail: bool = False,
        raise_exception: bool = False,
    ) -> AsyncContextManager[list[StructuredTool]]:
        """
        ...
        try:
            ...
            tasks = [
                asyncio.create_task(load_mcp_tools(c.session), name=name)
                for name, c in self._servername2clients.items()
            ]
