    ...
    def check_and_init(cls, data: dict) -> dict:
        ...
        if data["url"].endswith("/"):
            raise ValidationError(
                f"'url' should not end with '/'. Got '{data['url']}'."
            )
        if (cls.path.endswith("/") or not cls.path.startswith("/")) and cls.path != "":
            raise ValidationError(
                f"Class variable 'path' must start and Not end with '/'. Got '{cls.path}'."
            )
...
class ClientResponseSchema[DataType: dict](BaseModel):
    ...
    async def from_client_response(
        ...
        return await asyncio.to_thread(
            cls._from_client_response, response, content, info
        )

    @classmethod
    def _from_client_response(
        cls, response: ClientResponse, content: Any = None, info: str | None = None
    ):
        return cls(
...
class ClientRequestSchema[DataType: dict](BaseModel):
    ...
    method: Literal["GET", "POST", "PUT", "DELETE"] | None = Field(
        None,
...
async def get_client_response(
    client: BaseClient, request: ClientRequestSchema
    ...
    async with aiohttp.ClientSession() as session:
        async with session.request(
            **request.model_dump(mode="json", by_alias=True)
            ...
            logger.debug(
                f"{client.__class__.__name__} called request {request.method} {request.url}"
            )
            if not response.ok:
...
async def json_response_handler(
    response: ClientResponse, response_type: BaseModel | None = None
) -> ClientResponseSchema:
    content = await getattr(
        response, response_action[response.content_type]
    )()  # ex: response.json
    ...
    if issubclass(response_type, BaseModel) and response.ok:
        try:
            ...
            raise ValidationError(
                "Server return successfully but response type not match with the expected schema."
            )
    # noinspection PyTypeHints
    return await ClientResponseSchema[response_type].from_client_response(
        response, content
    )
...
def get_http_response(
    ...
    response_handler: Callable[
        [ClientResponse, ...], Coroutine[..., ..., ClientResponseSchema]
