...
class _RedisWrapperAbstract(BaseModel, ABC, _REDIS):
    ...
    def __getattr__(
        self, item
    ) -> Callable[..., Coroutine | Awaitable] | Awaitable | Any:
...
class RedisWrapperClient(_RedisWrapperAbstract, _REDIS):
    ...
    rwrapper: RedisWrapper | SentinelWrapper | ClusterWrapper = Field(
        discriminator="mode"
    )
    ...
    #     self,
    #     key: str,
    #     raise_on_redis_errors=False,
    #     auto_release_time: int = 10_000,
    #     num_extensions: int = 3,
    #     context_manager_blocking: bool = True,
    #     context_manager_timeout: float = -1,
    # ) -> AbstractAsyncContextManager[Redlock]:
    #     """Lock the key.
    #         Args:
    #                 key:
...
#
#                 print(
#                     f"the task {asyncio.current_task().get_name()} working on item #: {current_num}"
#                 )
#
